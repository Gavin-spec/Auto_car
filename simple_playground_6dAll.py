...
class Playground():
    ...
    def _readPathLines(self):
        try:
            ...
            self._setDefaultLine()
    def train(self):
        df = pd.read_csv('train6dAll.txt',header=None)[0].str.split(' ', expand=True)
        df = df.astype(float)
        ...
        X = (X - X_mean) / (X_std)
        self.model = RBFN(5, 100, 1, 0.02,20000)
        self.model.fit( X, y)
        self.X_mean = X_mean
        self.X_std = X_std
...
class UI:
    ...
    def update_playground(self):
        ...
        if not p.done:
            ...
            self.track_file.write(f"{car_pos.x}, {car_pos.y} ,{self.state[0]}, {self.state[1]}, {self.state[2]},{ p.car.angle }\n")
